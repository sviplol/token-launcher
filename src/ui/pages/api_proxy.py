...
class ApiProxyPage(QWidget):
    ...
    def _close_menus(self):
        ...
        「操作 ▾」菜单弹出后，定时器刷新重建表格行 → 弹出中的 QMenu 被 Qt 删除 →
        用户点击时事件处理到已释放对象。先 close 再重建可避免。
        """
    ...
    def _on_keys_header_sort(self, section: int):
        """上游 Key 表头排序：第一次正序，第二次倒序，第三次取消（恢复默认）"""
        if section >= self._keys_table.columnCount() - 1:
            return
        header = self._keys_table.horizontalHeader()
        if self._keys_sort_column == section and self._keys_sort_order == Qt.DescendingOrder:
            self._keys_sort_column = None
            self._keys_sort_order = Qt.AscendingOrder
        ...
        if self._keys_sort_column == section:
            self._keys_sort_order = Qt.DescendingOrder
        else:
            self._keys_sort_column = section
            self._keys_sort_order = Qt.AscendingOrder
        header.setSortIndicator(section, self._keys_sort_order)
        self._refresh_upstream_keys()

    def _on_subkeys_header_sort(self, section: int):
        """子 Key 表头排序：第一次正序，第二次倒序，第三次取消（恢复默认）"""
        if section >= self._subkeys_table.columnCount() - 1:
            return
        header = self._subkeys_table.horizontalHeader()
        if self._subkeys_sort_column == section and self._subkeys_sort_order == Qt.DescendingOrder:
            self._subkeys_sort_column = None
            self._subkeys_sort_order = Qt.AscendingOrder
            header.setSortIndicator(-1, Qt.AscendingOrder)
            self._refresh_sub_keys()
        ...
        if self._subkeys_sort_column == section:
            self._subkeys_sort_order = Qt.DescendingOrder
        else:
            self._subkeys_sort_column = section
            self._subkeys_sort_order = Qt.AscendingOrder
        header.setSortIndicator(section, self._subkeys_sort_order)
        self._refresh_sub_keys()
    ...
    def _refresh_upstream_keys(self, reload_from_disk=False):
        ...
        self._close_menus()  # 防弹出中菜单被重建删除（SIGSEGV 根因）
