...
class SettingsPage(QWidget):
    ...
    def apply_theme(self):
        """主题切换时刷新滚动区 viewport 背景（QPalette，防止白底残留）"""
        try:
            ...
            if hasattr(self, '_scroll_content'):
                self._scroll_content.setAutoFillBackground(True)
                pal2 = self._scroll_content.palette()
                pal2.setColor(QPalette.ColorRole.Window, QColor(bg))
                self._scroll_content.setPalette(pal2)
        except Exception:
            pass
