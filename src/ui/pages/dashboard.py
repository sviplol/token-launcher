...
def _current_theme_colors() -> dict:
    """获取当前主题颜色字典 — 接入极简黑白新主题（ui_theme: auto/light/dark），兼容旧键名"""
    from ..theme import resolve_colors
    c = resolve_colors()
    return {
        # 新键
        **c,
        # 旧键兼容（dashboard/其他页面内部用的键名全量映射）
        "bg_primary": c["bg"],
        "bg_secondary": c["bg2"],
        "bg_tertiary": c["bg3"],
        "bg_card": c["card"],
        ...
        "text_tertiary": c["text3"],
        "text_inverse": c["accent_text"],
        "accent": c["accent"],
        "accent_hover": c["accent_hover"],
        "accent_light": c["bg3"],
        "success": c["ok"],
        "success_light": c["ok_bg"],
        "warning": c["warn"],
        "warning_light": c["warn_bg"],
        "error": c["err"],
        "error_light": c["err_bg"],
        "border": c["border"],
        "border_light": c["border2"],
        "shadow": c["shadow"],
        "sidebar_bg": c["sidebar_bg"],
        "sidebar_text": c["sidebar_text"],
        "sidebar_logo": c["text"],
        "sidebar_border": c["border"],
        "sidebar_active": c["sidebar_active_bg"],
        "sidebar_active_text": c["sidebar_active_text"],
        "sidebar_hover": c["bg3"],
        "sidebar_hover_text": c["text"],
    }
...
class DashboardPage(QWidget):
    ...
    def _setup_ui(self):
        ...
        self._guide_card.setObjectName("card")
        guide_layout = QVBoxLayout(self._guide_card)
        guide_layout.setContentsMargins(24, 20, 24, 20)
        guide_layout.setSpacing(10)

        guide_title = QLabel("欢迎使用 Token接入器")
        guide_title.setStyleSheet("font-size: 20px; font-weight: 800; background: transparent; border: none;")
        guide_layout.addWidget(guide_title)

        guide_text = QLabel(
            "还没有账号？只需两步开始使用：\n\n"
            "  1️⃣ 点击右上角「🎫 输入卡密添加账号」，粘贴卡密即可导入账号\n"
            "  2️⃣ 前往「⚡ 一键接入」页，点击启动，WorkBuddy / CodeBuddy 自动接入\n\n"
            "导入账号后此处将显示统计概览。")
        guide_text.setWordWrap(True)
        guide_text.setStyleSheet("font-size: 14px; color: #6B7280; background: transparent; border: none; line-height: 1.8;")
        guide_layout.addWidget(guide_text)
        content_layout.addWidget(self._guide_card)
    ...
    def _range_btn_style_active(self) -> str:
        """选中状态按钮样式（跟随缩放，极简黑白反色：黑主题白底黑字，白主题黑底白字）"""
        c = self._colors
        s = self._scale
        pad_v = int(6 * s)
        pad_h = int(16 * s)
        font_size = int(13 * s)
        return (
            f"QPushButton {{ background-color: {c['accent']}; color: {c['accent_text']}; "
            f"border: none; padding: {pad_v}px {pad_h}px; border-radius: 6px; font-size: {font_size}px; font-weight: 700; }}"
        )
    ...
    def _refresh_data(self):
        ...
        # 新手引导卡：有账号就隐藏
        if hasattr(self, '_guide_card'):
            self._guide_card.setVisible(total == 0)
