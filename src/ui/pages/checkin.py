        ...
        try:
            ...
            if result["success"]:
                if result.get("already"):
                    ...
                    total_remaining = account.quota.credits_remaining
                    return (account.display_name, "success",
                            f"签到成功！+{credit}积分 (连续{account.checkin.streak_days}天，当前可用{total_remaining:.0f}积分)")
            else:
...
class CheckinPage(QWidget):
    ...
    def _setup_ui(self):
        ...
        self._stat_checked.setStyleSheet("font-size: 14px; font-weight: 700; color: #0A7D33;")
        stats_row.addWidget(self._stat_checked)

        self._stat_unchecked = QLabel("⏳ 未签到: 0")
        self._stat_unchecked.setStyleSheet("font-size: 14px; font-weight: 700; color: #9A6B00;")
        stats_row.addWidget(self._stat_unchecked)

        self._stat_monthly_credits = QLabel("💰 本月积分: 0")
        self._stat_monthly_credits.setStyleSheet("font-size: 14px; font-weight: 700;")
        stats_row.addWidget(self._stat_monthly_credits)

        stats_row.addStretch()

        # 刷新状态按钮
        self._btn_refresh_status = QPushButton("🔄 刷新签到状态")
        ...
        # 停止按钮
        self._btn_stop = QPushButton("⏹ 停止签到")
    ...
    def _on_checkin_finished(self, results: dict):
        ...
        try:
            total_remaining = sum(
                acc.quota.credits_remaining for acc in self._accounts
                if acc.quota.credits_total > 0
            )
            gained = results.get('success', 0) * 0  # 积分增量在明细里逐条显示
            self._append_log(
                f"💰 签到后账号总可用积分：{total_remaining:.0f} 积分"
                f"（共 {sum(1 for acc in self._accounts if acc.quota.credits_total > 0)} 个已查询账号）"
            )
        except Exception:
            pass
