...
# 未找到客户端路径的错误前缀（UI 识别此前缀弹「手动指定路径」引导，cockpit 同款错误码）
APP_PATH_NOT_FOUND_PREFIX = "APP_PATH_NOT_FOUND:"
...
# ---------------------------------------------------------------------------
# WorkBuddy 客户端路径解析（复刻 cockpit resolve_workbuddy_launch_path 三层）
# 1) 用户手动指定的路径（config.workbuddy_app_path，最高优先）
# 2) 自动探测候选（detect_workbuddy_exec_path）
# 3) macOS .app 归一化：截 .app 根 → 试 WorkBuddy/Electron → 目录扫描兜底
# 探测成功即写回配置固化，下次直接走配置；用户手动改过的路径不被探测覆盖。
# ---------------------------------------------------------------------------

def _app_config_path() -> Path:
    """本应用自己的配置文件（与日志目录同根）。"""
    return Path.home() / ".flash-connector" / "config.json"
...
def get_workbuddy_app_path() -> str:
    """用户手动指定的 WorkBuddy 客户端路径（未指定则空）。"""
    return str(_load_app_config().get("workbuddy_app_path") or "").strip()


def set_workbuddy_app_path(path: str) -> None:
    """保存用户手动指定的 WorkBuddy 客户端路径（原子写）。"""
    cfg = _load_app_config()
    cfg["workbuddy_app_path"] = (path or "").strip()
    p = _app_config_path()
...
def _normalize_custom_path(value: str) -> str:
    """清洗用户手填的路径：去空白、去引号包裹、去 Windows 长路径前缀 \\\\?\\。"""
    v = (value or "").strip().strip('"')
    if _is_windows() and v.startswith("\\\\?\\"):
        v = v[4:]
    return v


def _normalize_macos_app_root(path_str: str) -> str:
    """把 xxx.app/Contents/MacOS/WorkBuddy 截断成 xxx.app 根（cockpit normalize_macos_app_root）。"""
    idx = path_str.find(".app")
    if idx >= 0:
        return path_str[: idx + 4]
    return path_str


def _find_macos_exec(app_root: str) -> str:
    """在 .app 的 Contents/MacOS 里找可执行二进制（cockpit 同款：先名字、再目录扫描兜底）。"""
    macos_dir = Path(app_root) / "Contents" / "MacOS"
    for name in ("WorkBuddy", "Electron"):
        candidate = macos_dir / name
        if candidate.is_file():
            return str(candidate)
    try:
        entries = sorted(macos_dir.iterdir())
    except OSError:
        return ""
    fallback = ""
    for entry in entries:
        ...
        if "crashpad" in fname or "helper" in fname:
            continue
        if "workbuddy" in fname or fname == "electron":
            return str(entry)
        if not fallback:
            fallback = str(entry)
    return fallback
...
def _resolve_workbuddy_app_root(path_str: str) -> str:
    """把用户给的/探测到的路径归一化成可启动路径：
    - macOS：截 .app 根，校验 Contents/MacOS 里有可执行二进制，返回 .app 根
    - Windows：直接要求文件存在，返回 exe 路径
    返回 "" 表示该路径不可用。
    """
    path = Path(path_str)
    if _is_macos():
        root = _normalize_macos_app_root(str(path))
        if Path(root).is_dir() and _find_macos_exec(root):
            return root
        return ""
    if path.is_file():
        return str(path)
...
def resolve_workbuddy_app_root() -> str:
    """解析 WorkBuddy 客户端可启动路径（cockpit resolve_workbuddy_launch_path 同款三层）。

    1. 用户手动指定（配置 workbuddy_app_path）优先；指定了但找不到 → 清除该配置并回落自动探测，
       避免固化值失效（如客户端挪了位置）后永远堵死在手动分支
    2. 自动探测候选；命中后写回配置固化，下次直接走配置
    3. 都没有 → 返回 ""
    """
    custom = _normalize_custom_path(get_workbuddy_app_path())
    if custom:
        resolved = _resolve_workbuddy_app_root(custom)
        if resolved:
            return resolved
        # 手动指定失效：清掉配置回落自动探测，不再堵死
        set_workbuddy_app_path("")
        logger.warning("[切号] 手动指定的 WorkBuddy 路径已失效，已清除配置并回落自动探测: %s", custom)

    for candidate in workbuddy_app_candidates():
        resolved = _resolve_workbuddy_app_root(candidate)
        if resolved:
            # 探测成功 → 固化到配置，以后不再依赖探测
            if get_workbuddy_app_path() != candidate:
                set_workbuddy_app_path(candidate)
            return resolved
    return ""
...
def switch_to_workbuddy(account: Account, keep_sessions: bool = False) -> str:
    ...
    app_root = resolve_workbuddy_app_root()
    if not app_root:
        candidates = workbuddy_app_candidates()
        lines = "\n".join(f"  · {c}" for c in candidates)
        raise SwitchError(
            f"{APP_PATH_NOT_FOUND_PREFIX}workbuddy\n"
            "未找到 WorkBuddy 客户端，已自动检查以下位置：\n"
            f"{lines}\n\n"
            "如果你的 WorkBuddy 装在其它位置，请点击「手动指定路径」选择客户端。"
        )
