...
def check_api_key_chat_status(api_key: str, attempts: int = 3, uid: str = "") -> dict:
    """Probe whether a WorkBuddy API key can start a chat request.

    与网页端检测（card_lib.card_check_account_status）完全同款：
    X-User-Id + X-Domain + 浏览器UA + zh Accept-Language + max_tokens=1 最小探测。
    实测缺这些头 → APISIX 安全策略 400 code=11128 request illegal（百分百失败）。
