...
# ─── 支持的模型列表（已实测验证可用 + WorkBuddy v5.4.7 三档模型）───
SUPPORTED_MODELS = [
    # WorkBuddy 三档（v5.4.7 新增，替代旧 auto 单档）
    "fast-model", "balanced-model", "deep-model",
    # 兼容旧 auto（客户端旧版仍可能发 auto）
    "auto",
    # DeepSeek 系列
...
MODEL_ID_ALIASES = {
    ...
    "kimi-k3-1": "kimi-k3",  # kimi-k3-1 别名映射到 kimi-k3
}

MODEL_CONTEXT_LENGTHS = {
    # WorkBuddy 三档模型（v5.4.7）
    "fast-model": 200000,              # 200K
    "balanced-model": 200000,          # 200K
    "deep-model": 200000,              # 200K
    # 兼容旧 auto
    "auto": 168000,                    # 官方虚拟模型，服务端动态选模型
    # DeepSeek 系列
    ...
    "hunyuan-2.0-instruct": 128000,    # 128K
}
...
MODEL_MAX_OUTPUT_TOKENS = {
    # WorkBuddy 三档（v5.4.7：统一 48000）
    "fast-model": 48000,
    "balanced-model": 48000,
    "deep-model": 48000,
    "auto": 131072,
    "glm-5.2": 131072,
    "glm-5.1": 131072,
...
# 进程级稳定的 ACP 连接 id（真实客户端一次连接一个，严格 uuid v4 格式：
# 第三段 4 开头、第四段 8/9/a/b 开头，与抓包值 f3693cb7-9b09-4000-b6d4-... 同形）
_ACP_CONNECTION_ID = (
    f"{secrets.token_hex(4)}-{secrets.token_hex(2)}-"
    f"4{secrets.token_hex(2)[1:]}-{secrets.choice('89ab')}{secrets.token_hex(2)[1:]}-"
    f"{secrets.token_hex(6)}"
)


def _build_workbuddy_relay_headers(api_key: str, session_id: str = "") -> dict:
    """Build the upstream headers for WorkBuddy API key relay.

    与真实 WorkBuddy CLI 全量对齐（2026-08-09 经 8003 中转实测抓包，非手编）：
    - 静态身份头原样照抄抓包值；
    - X-Request-ID 与 X-Conversation-Message-ID 同值（真实行为如此），每请求随机；
    - trace 头（traceparent/b3/X-B3-*/X-Trace-ID）成对生成、互相关联；
    - X-Conversation-ID 按会话 hash 派生稳定 uuid（同会话不变，像真实客户端）；
    - X-User-Id 从 JWT sub 解码，保证与 token 同账号（ck_ 卡密非 JWT，不携带）。
    注意：TLS 指纹（python OpenSSL vs Node BoringSSL）无法在此层伪造。
    """
    request_id = secrets.token_hex(16)          # 32hex
    conv_request_id = secrets.token_hex(16)
    ...
    headers = {
        ...
        "X-IDE-Version": "5.3.8",
        "X-Private-Data": "true",
        "acp-connection-id": _ACP_CONNECTION_ID,
        "x-codebuddy-request": "1",
        "X-Request-ID": request_id,
        "X-Conversation-Message-ID": request_id,
        "X-Conversation-Request-ID": conv_request_id,
        "traceparent": f"00-{trace_id}-{span_id}-01",
        "b3": f"{trace_id}-{span_id}-1-{parent_span}",
        "X-B3-TraceId": trace_id,
        "X-B3-ParentSpanId": parent_span,
        "X-B3-SpanId": span_id,
        "X-B3-Sampled": "1",
        "X-Trace-ID": trace_id,
...
def _decode_jwt_sub(token: str) -> str:
    """解码 JWT payload 的 sub（即账号 uid；不验签，仅本地读取；失败返回空串）"""
    try:
        parts = token.split(".")
        if len(parts) != 3:
            return ""
...
def _build_workbuddy_relay_body(client_body: dict) -> tuple[dict, dict]:
    ...
    body["model"] = body.get("model") or "fast-model"
...
def _responses_to_chat_request(client_body: dict) -> tuple[dict, dict]:
    chat_body = {
        "model": client_body.get("model") or "fast-model",
...
class ProxyDatabase:
    ...
    def __init__(self, data_dir: str = ""):
        import os
        if not data_dir:
            data_dir = os.path.expanduser("~/.flash-connector")
        ...
        with self._lock:
            ...
            for k in keys:
                if k.get("key_id") == key_id:
                    ...
                    if credits:
                        k["relay_credits"] = round(float(k.get("relay_credits", 0.0) or 0.0) + credits, 4)
                    break
            self._update_daily_stats("relay", key_id, prompt_tokens,
                                     completion_tokens, total_tokens, cached_tokens, credits)
            self._save()
    ...
    def _handle_chat_completions(self, request_data_override: dict = None,
        ...
        while len(tried_key_ids) < MAX_RETRY_KEYS:
            ...
            try:
                ...
                req_headers = _build_workbuddy_relay_headers(
                    upstream_api_key, session_id=self.router._get_session_id(request_data))
