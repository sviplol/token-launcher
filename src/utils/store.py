...
def _get_db_path() -> Path:
    """获取数据库路径 — Token接入器独立目录，不碰 antigravity-tools"""
    app_dir = Path.home() / ".flash-connector"
    try:
...
def save_account(account: Account):
    """保存账号到数据库（UPSERT：更新时不覆盖 account_group——卡密来源标记防丢失）"""
    conn = get_connection()
    try:
        conn.execute("""
            ...
            ON CONFLICT(uid) DO UPDATE SET
                ...
                checkin_rewards=excluded.checkin_rewards,
                daily_credit=excluded.daily_credit,
                total_credits=excluded.total_credits,
                hourly_suggestions=excluded.hourly_suggestions,
                hourly_suggestions_limit=excluded.hourly_suggestions_limit,
                weekly_chat=excluded.weekly_chat,
                weekly_chat_limit=excluded.weekly_chat_limit,
                credits_remaining=excluded.credits_remaining,
                credits_total=excluded.credits_total,
                reset_time=excluded.reset_time,
                quota_last_updated=excluded.quota_last_updated,
                quota_last_error=excluded.quota_last_error,
                quota_last_error_at=excluded.quota_last_error_at,
